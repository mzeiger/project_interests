...
CREATE TABLE `project` (
  ...
  `estimatedTime` varchar(200) NOT NULL,
...
(299, 'Margarita @ Pine Creek Wine Tasting', 160, 'Dirk Stamp, Rich Strom', 'One evening, usually in November', 'Volunteers assist the project manager in public relations, preparation for, and execution of the project and attend in support.', 'In the fall of each year, the proprietor of The Wine Seller in Monument works with Margarita @ Pine Creek to stage a wine tasting event in support of Monument Hill Kiwanis projects and granting.  Members may volunteer to assist with advertising and by attending the event in Club vests to provide public exposure for our Club and mission.', 0),
...
(307, 'Director, Service Programs', 2070, 'TBD', 'Substantial investment in time', 'Responsible for oversight of all service activities of the Club.  Recruits project managers and deputies for five current service projects', '', 0),
...
ALTER TABLE `interest`
  ADD UNIQUE KEY `idx_person_project` (`personId`,`projectId`);
...
ALTER TABLE `project`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `idx_projectName` (`id`),
  ADD UNIQUE KEY `idx_position` (`position`);
